...
The instruments of the preceding sections are components; this section assembles them into a single loop that a working researcher can run, and states the status-upgrade rules that keep the loop's outputs meaning what they say. The protocol generalizes and anonymizes a workflow this library's private research vault runs in practice; nothing here depends on the private data, only on its shape.
