...
All
numbers reproduce deterministically from the versioned code (seed~42) and the author bears full
...
both sub-periods (Diebold-Mariano $p=0.0008$, but it \emph{fails} the pre-registered Clark-West gate for this near-nested pair, $p\approx0.03>0.0083$, so it is reported as exploratory, not confirmatory). A lag diagnostic shows the forecast is
...
\texttt{PREREGISTRATION-FORECAST-EN.md} (note QR002, frozen 2026-08-15): the single
...
the drawdown advantage shrinks toward parity, and single-market VMG matches the combination on
Sharpe --- which is scale-invariant, so the vol-matching adds nothing to \emph{that} comparison;
only a modest Calmar difference survives the common-volatility normalization. We therefore report
no tradeable edge.

\emph{Cost stress and the graveyard.} VMG and VIXTS survive five basis points --- though VIXTS survives the \emph{cost} stress, not its own hypothesis: the pre-registered Q015 (VIX-term-structure sizing improving Calmar by $+0.05$ to $+0.15$) reaches only $\Delta$Calmar~$\approx+0.023$ at its best grid point, with five of six grid points negative, so it misses its pre-registered band and is recorded \emph{not supported}; and Q012 (TSMOM, correlation $<0.5$ to VMG) measures $0.751$, violating its own condition. The cost-fragile
...
it clears the DM gate but \emph{not} the pre-registered Clark-West gate (the honest test for this near-nested pair), so it stays exploratory and unconfirmed absent a live log. \emph{(2)~The strategy is beta management at
...
\vfill
\sloppy\noindent\small\emph{Working paper v1.1, \today. Reproduction: the two pre-registration
