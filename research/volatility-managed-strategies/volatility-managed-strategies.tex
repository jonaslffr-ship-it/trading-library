...
$90\,\%$ MCS becomes $\{$Lasso, ElasticNet, HAR-CJ$\}$ and \textbf{HAR-IV is excluded at $p=0.013$}.
We report this openly --- the feature family genuinely displaces HAR-IV on QLIKE --- but do not build
the sizing rule on it, precisely because that QLIKE win rides the $-65\,\%$ MSE-$R^2$ of Lasso~(c).
...
(PBO~$=0.79$ for HAR-IV, $0.75$ for log-HAR; \texttt{thesis\_validation.csv}),\cf{Consistent with Cederburg, O'Doherty, Wang \& Yan (2020).} yet the family, honestly re-selected,
...
\emph{Cost stress and the graveyard.} VMG and VIXTS survive five basis points; the cost-fragile
