...
The direction check first: over-coverage (81% > 68%) means the priced band was systematically *wider* than what the market subsequently did — implied volatility above realized volatility. That is the same direction as Section 4.3's single-day illustration and the VRP literature, so the pieces agree; had the share come in *below* 68%, it would have contradicted the premium claim and this section would say so. One caveat sizes the claim honestly, though: 68.3% is the *normal* benchmark, and daily returns are fat-tailed, so even a fairly priced band sits above it. The right leptokurtic baseline is *conditional*, however — the test standardizes each day by its own VIX-implied sigma, so the comparison distribution is the VIX-standardized daily return, whose excess kurtosis is only about 1.4, not the ~14 of raw undivided returns. A fairly priced band of that conditional shape already covers about 71.9%. Measured against that correct baseline, of the 13.1-point over-coverage roughly 3.7 points are fat tails and the remaining ~9.5 are the variance risk premium — so the premium is the *larger* part, not the residual. (An earlier revision compared against the *unconditional* Student-t(4)/t(5) coverage of 77.0%/74.7%, which uses the kurtosis of raw returns rather than the VIX-standardized ones the test actually scores, and so understates the premium — see ERRATA C1.) What the figure adds is texture: the rolling one-year share never fell below 68.7% (reached in December 2022, at the end of a grinding bear market — the one period where the VIX band was merely fair rather than generous) and stretched to 92.5% in the placid markets of late 2021, where the VIX overpriced daily movement spectacularly.
...
- **"About 68% of days stay inside the ±1σ expected move."** Empirically 81.4% did (Section 7.4) — mostly because the band is built from a premium-inflated price (about 9.5 of the 13.1 excess points), with fat tails adding the rest (about 3.7), measured against the correct VIX-standardized baseline. Naive theory quotes 68%; measurement says otherwise — and neither number makes selling the band free money, because the misses cluster in the worst tapes.
